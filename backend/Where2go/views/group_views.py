...
class LeaveGroupView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Выход из группы", responses={200: "Вы покинули группу"}
    )
    def post(self, request, group_id):
        group = Group.objects.get(id=group_id)
        user = request.user
...
class ManageGroupView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "name": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Новое имя группы"
                )
            },
        ),
...
class DeleteGroupView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Удаление группы",
        responses={204: "Группа успешно удалена", 403: "Нет прав для удаления группы"},
    )
    def delete(self, request, group_id):
